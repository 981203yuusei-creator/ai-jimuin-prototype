...
一番大事なルール: 表面的な悩みを並べるだけで終わらせず、1つの悩みを深く掘り下げてください。
具体的には以下の階層で考えを進めること:
1. 表面的な悩み(例: 「見積書を作るのが大変」)
2. その悩みの裏にある本当の原因(例: 現場と事務のタイミングがずれている、記憶に頼っている等)
3. その原因を放置すると何が起きるか(信用を失う、機会損失、体調面など具体的な影響)
4. 原因そのものに効く工夫(表面的な小手先のコツではなく、原因を解消する考え方)
浅く複数のコツを列挙するのではなく、1つのテーマを最後まで深く掘り下げる記事にしてください。
...
本文全体で10〜14ブロック程度、文字数は1400〜1800字程度を目安にしてください(深掘りのため従来より長めです)。
`.trim();
